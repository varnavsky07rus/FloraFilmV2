package com.alaka_ala.florafilm.ui.util.api.vcdn.models;

import java.io.Serializable;
import java.util.List;

public class SerialVCDN implements Serializable {

    private final List<Season> seasons;

    public SerialVCDN(List<Season> seasons) {
        this.seasons = seasons;
    }

    public List<Season> getSeasons() {
        return seasons;
    }


    public static class Season implements Serializable {

        public String getSeason_name() {
            return season_name;
        }

        public List<Episode> getEpisodes() {
            return episodes;
        }

        private final String season_name;
        private final List<Episode> episodes;

        public Season(String season_name, List<Episode> episodes) {
            this.season_name = season_name;
            this.episodes = episodes;
        }
    }

    public static class Episode implements Serializable {
        public String getEpisodes_name() {
            return episodes_name;
        }

        public String getPoster() {
            return poster;
        }

        public List<Media> getMedia() {
            return media;
        }

        private final String episodes_name;
        private final String poster;
        private final List<Media> media;

        public Episode(String episodes_name, String poster, List<Media> media) {
            this.episodes_name = episodes_name;
            this.poster = poster;
            this.media = media;
        }



    }

    public static class Media implements Serializable {
        private final int translation_id;
        private final String translation_name;
        private final String playlist;
        private final int max_quality;

        public int getMax_quality() {
            return max_quality;
        }

        public String getPlaylist() {
            return playlist;
        }

        public String getTranslation_name() {
            return translation_name;
        }

        public int getTranslation_id() {
            return translation_id;
        }

        public Media(int translation_id, String translation_name, String playlist, int max_quality) {
            this.translation_id = translation_id;
            this.translation_name = translation_name;
            this.playlist = playlist;
            this.max_quality = max_quality;
        }

    }


}
